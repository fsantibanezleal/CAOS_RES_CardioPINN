...
def run() -> dict:
    ...
    for frac in NOISE:
        for seed in SEEDS:
            ...
            row = {
                ...
                "seed": seed,
                "soft": {
                    "div_nd": round(B.divergence_grid(vs, mask, h) / norm, 5),
                    "vel_err": round(_vel_error(vs, v_true, mask), 4),
                    **B.pressure_error(ps, p_true, mask),
                },
                "curl": {
                    "div_nd": round(B.divergence_grid(vc, mask, h) / norm, 5),
            ...
            print(
                ...
                f"dropErr={row['curl']['drop_err_mmHg']:.3f}",
                flush=True,
            )
    ...
    verdict = {
        ...
        "advance": bool(
            med_curl_div < 0.25 * med_soft_div and curl_wins_pressure >= n / 2 and curl_wins_velocity >= n / 2
        ),
    }
