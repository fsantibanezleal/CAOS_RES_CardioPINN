...
from cardiopinnlab.real.flow4d_ppe import RHO, MU, PA_PER_MMHG, solve_ppe_precomputed
...
def converging_duct(h: float = 0.0015):
    ...
    xs = np.arange(-Rd, Rd + h, h)
    ys = xs.copy()
    zs = np.arange(0, Lz + h, h)
...
def pressure_error(p_rec: np.ndarray, p_true: np.ndarray, mask: np.ndarray) -> dict:
    ...
    return {
        "corr": round(corr, 4),
        "scale": round(scale, 4),
        "true_drop_mmHg": round(true_drop, 3),
        "rec_drop_mmHg": round(rec_drop, 3),
        "drop_err_mmHg": round(abs(rec_drop - true_drop), 4),
        "rmse_mmHg": round(rmse, 4),
    }
...
def _curl_nd(A: torch.Tensor, xc: torch.Tensor) -> torch.Tensor:
    ...
    g = [
        torch.autograd.grad(
            A[:, i], xc, torch.ones(n, device=xc.device), create_graph=True, retain_graph=True
        )[0]
        for i in range(3)
    ]  # g[i][:,j] = dA_i/dx_j
    vx = g[2][:, 1] - g[1][:, 2]
...
class CurlField:
    ...
    def velocity(self, pts_m: np.ndarray, batch: int = 20000) -> np.ndarray:
        ...
        for s in range(0, len(pts_m), batch):
            xc = torch.tensor(
                (pts_m[s : s + batch] - self.c0) / self.L,
                dtype=torch.float32,
                device=self.device,
                requires_grad=True,
            )
        ...
        for s in range(0, N, batch):
            ...
            for i in range(3):
                Ji = torch.autograd.grad(
                    v_nd[:, i], xc, torch.ones(n, device=self.device), retain_graph=True
                )[0]  # d v_nd_i / d x_nd [n,3]
                J[:, i, :] = Ji * (sU / sL)
            v_si = v_nd.detach() * sU
            Jd = J.detach()
