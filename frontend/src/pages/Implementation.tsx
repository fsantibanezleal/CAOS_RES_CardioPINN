  ...
  return (
    <div className="fig-svg wide">
      <svg viewBox="0 0 760 250" role="img" style={{ width: '100%', height: 'auto' }}>
        ...
        <text x="22" y="46" fill="var(--muted)" fontSize="11.5">{pick(lang, 'OFFLINE: the physics (local machine, never in CI or the web)', 'OFFLINE: la física (maquina local, nunca en CI ni la web)')}</text>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Cada resultado se computa offline y se compromete como un trace JSON compacto; la web estatica solo lo lee (sin servidor, sin modelo en el navegador). El carril ECGi es NumPy/SciPy puro en CPU; el carril de flujo 4D entrena PINNs pequeñas en una GPU local. Ambos son deterministas dado el seed y se validan en CI contra un artefacto comprometido.')}</p>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Los datasets EDGAR tienen nombres de campo y disposiciones de malla por laboratorio, así que una sola configuracion guia el cargador. Utah guarda potenciales en un struct ts de MATLAB con campo potvals y mallas como structs node/face; Maastricht guarda arreglos crudos hartpots (corazon) y lichaampots (cuerpo) con mallas hart/lichaam. El cargador lee cada caso por su configuracion, descarta cuadros con NaN, y devuelve una forma comun: potenciales corporales, potenciales cardiacos, las dos nubes de nodos, y la triangulacion cardiaca.')}</p>
          <p>{pick(lang,
          ...
          <Callout>{pick(lang, 'The completeness floor is enforced: the catalogue bake must contain >=2 datasets and >=4 beats, so a partial bake cannot silently shrink the catalogue.', 'El piso de completitud se impone: el precálculo del catalogo debe contener >=2 datasets y >=4 latidos, así que un precálculo parcial no puede encoger el catalogo en silencio.')}</Callout>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Se implementan dos operadores directos sobre la geometría real. El default es un nucleo de capa simple (fuente puntual) evaluado en las posiciones reales de electrodos y normalizado por filas, con una ganancia escalar calibrada una vez en la primera mitad del latido y luego fijada. El segundo es un operador de elementos de contorno completo: ángulos solidos exactos de triangulo (Van Oosterom-Strackee) para la doble capa, integrales de 1/r por triangulo para la capa simple, el salto c(p) en una diagonal deflactada, y la corriente normal cardiaca eliminada para dar la matriz de transferencia.')}</p>
          ...
          <Sym items={[['g', pick(lang, 'calibrated scalar gain', 'ganancia escalar calibrada')], ['x^b, x^h', pick(lang, 'body / heart electrode positions', 'posiciones de electrodo cuerpo / corazon')], ['D, G', pick(lang, 'double-layer (solid angle) / single-layer (1/r) blocks', 'bloques doble capa (ángulo solido) / capa simple (1/r)')], ['Z', pick(lang, 'the BEM transfer matrix', 'la matriz de transferencia BEM')]]} />
          <Callout>{pick(lang, 'The BEM is analytic-gated (concentric spheres: correlation 1.00, error halving per mesh refinement) but on the coarse real electrode geometry it does not beat the calibrated single-layer, so the single-layer stays the default. A null result, reported not hidden.', 'El BEM tiene prueba analitica (esferas concentricas: correlación 1.00, error a la mitad por refinamiento) pero sobre la geometría gruesa real no supera a la capa simple calibrada, así que la capa simple sigue de default. Un resultado nulo, reportado no oculto.')}</Callout>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'La reconstrucción resuelve un problema de minimos cuadrados regularizado. Dos penalizaciones se comparan en su mejor lambda por oraculo (barrido en 30 valores log-espaciados, el que minimiza el error real contra la jaula, para juzgar cada método en su mejor version): una penalizacion Tikhonov de orden cero (identidad) y una penalizacion de Laplaciano de grafo construida de la triangulacion real. Un ensemble profundo de K=6 reconstrucciones sobre realizaciones de ruido (sigma = 2% de la desviacion de los datos) da una dispersion por nodo, recalibrada por una temperatura para que la banda 2-sigma coincida con el error real.')}</p>
          <Equation tex={String.raw`\hat\phi=(A^\top A+\lambda^2 L^\top L)^{-1}A^\top\phi_{\text{body}}, \qquad \bar\phi=\tfrac1K\textstyle\sum_k\hat\phi^{(k)},\;\; s=\tau\cdot\mathrm{std}_k\,\hat\phi^{(k)}`}
            caption={pick(lang, 'The closed-form regularized solve; the ensemble mean is the reconstruction and the recalibrated spread s is the per-node uncertainty.', 'La solucion regularizada en forma cerrada; la media del ensemble es la reconstrucción y la dispersion recalibrada s es la incertidumbre por nodo.')} />
          <Sym items={[['λ', pick(lang, 'oracle-best regularization strength', 'mejor fuerza de regularizacion por oraculo')], ['L', pick(lang, 'identity (Tikhonov) or mesh graph-Laplacian', 'identidad (Tikhonov) o Laplaciano de grafo de malla')], ['K, τ', pick(lang, 'ensemble size (6) and recalibration temperature', 'tamaño del ensemble (6) y temperatura de recalibracion')]]} />
          <Callout>{pick(lang, 'Results, ensemble mean (the shipped reconstruction, vs the REAL heart cage): human sinus RE 0.67 / CC 0.72, PVP 0.67 / 0.71, AVP 0.57 / 0.80; dog sinus 0.59 / 0.73; node-UQ ~0.90. The zeroth-order Tikhonov baseline it is compared against is modestly more accurate (sinus 0.65 / 0.72, PVP 0.58 / 0.80, AVP 0.54 / 0.85, dog 0.54 / 0.78). Same pipeline, no per-heart retuning, so the numbers measure a method.', 'Resultados, media del ensemble (la reconstrucción desplegada, vs la jaula REAL): humano sinusal RE 0.67 / CC 0.72, PVP 0.67 / 0.71, AVP 0.57 / 0.80; perro sinusal 0.59 / 0.73; UQ nodo ~0.90. La linea base Tikhonov de orden cero con la que se compara es modestamente mas precisa (sinusal 0.65 / 0.72, PVP 0.58 / 0.80, AVP 0.54 / 0.85, perro 0.54 / 0.78). Mismo pipeline, sin reajuste, así que los números miden un método.')}</Callout>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'La serie DICOM de flujo 4D (Philips) guarda, por cuadro cardiaco, una imagen de magnitud y tres imagenes de velocidad por contraste de fase a lo largo de los ejes RL, AP y FH del paciente. La fase de 12 bits mapea a velocidad por el rescale DICOM: velocidad = (slope*px + intercept)/4096 * venc, así que el rango reescalado abarca mas/menos el venc (120 cm/s). Los centros de voxel vienen de ImagePositionPatient / ImageOrientationPatient / PixelSpacing, así que velocidad y geometría comparten el marco del paciente. Las velocidades sobre el venc pliegan la fase; los voxeles plegados se detectan contra un estimador local robusto y se desdoblan por dos veces el venc antes de reconstruir.')}</p>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Una red espacio-temporal v(x,y,z,t) se entrena sin divergencia sobre todo el ciclo cardiaco, así que la fuente de la Poisson de presion Y la aceleración no estacionaria dv/dt son ambas analiticas (autograd en el tiempo), reemplazando una diferencia finita de tres cuadros. Se prueba en un flujo analitico de Poiseuille variable en el tiempo cuyo dw/dt exacto se conoce: la red lo recupera con correlación 0.995. En el escaneo real el termino analitico lleva el rango de presion relativa de 14.87 mmHg inflado (diferencia finita ruidosa) a un pequeño y fisiologico 0.79 mmHg, del mismo orden que la estimación clinica de Bernoulli (2.51 mmHg).')}</p>
          ...
          <Callout>{pick(lang, 'A velocity-noise ensemble (5% of the venc) moves the pressure by under 0.01 mmHg: the denoiser makes the pressure robust to velocity noise, so the dominant uncertainty is instead the absent gold standard + segmentation + unsteady approximation, which an ensemble cannot quantify. Reported as a scalar, not a misleading uniform ~0 UQ field.', 'Un ensemble de ruido de velocidad (5% del venc) mueve la presion en menos de 0.01 mmHg: el suavizador hace la presion robusta al ruido, así que la incertidumbre dominante es el patron de oro ausente + segmentación + aproximación no estacionaria, que un ensemble no puede cuantificar. Reportado como escalar, no como un campo UQ uniforme ~0 enganoso.')}</Callout>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Un validador y pytest corren en CI con pisos duros para que un precálculo parcial o no fisiologico falle en vez de desplegarse en silencio: el catalogo ECGi debe tener al menos dos datasets y cuatro latidos con todos los campos y metricas; el rango de presion de flujo 4D debe ser fisiologico (bajo 60 mmHg) y la velocidad pico en rango fisiologico. Cada motor tiene además una prueba de gate analitico que debe pasar antes de confiar en datos reales.')}</p>
