    ...
    },
    {
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            'Atrial fibrillation and ventricular tachycardia are common and dangerous. Atrial fibrillation now affects on the order of 52 million people worldwide, and it is not benign: the Framingham cohort showed it confers a near fivefold excess risk of stroke, independent of other factors, with the attributable risk climbing steeply into old age (from about 1.5% at age 50-59 to about 23.5% at 80-89). Ventricular tachycardia sits upstream of a large share of sudden cardiac deaths. For both, the definitive treatment when drugs fail is catheter ablation: burning or freezing the small piece of tissue that starts or sustains the arrhythmia. Ablation is fundamentally a localization problem. You cannot destroy a source you cannot find.',
            'La fibrilacion auricular y la taquicardia ventricular son frecuentes y peligrosas. La fibrilacion auricular afecta hoy del orden de 52 millones de personas en el mundo, y no es benigna: la cohorte de Framingham mostro que confiere un riesgo casi quintuplicado de accidente cerebrovascular, independiente de otros factores, con el riesgo atribuible subiendo abruptamente con la edad (de cerca del 1.5% a los 50-59 anos a cerca del 23.5% a los 80-89). La taquicardia ventricular esta aguas arriba de una gran parte de las muertes cardiacas subitas. Para ambas, el tratamiento definitivo cuando fallan los farmacos es la ablacion con cateter: quemar o congelar el pequeno trozo de tejido que inicia o sostiene la arritmia. La ablacion es en esencia un problema de localizacion. No puedes destruir una fuente que no puedes encontrar.')}</p>
          <p>{pick(lang,
            'The tool used to read the heart electricity, the standard 12-lead ECG, cannot find it. Twelve leads are really only eight independent signals: a very low-dimensional projection of a three-dimensional electrical field onto a few points on the skin. That is enough to diagnose that an arrhythmia exists and to guess a broad region of origin from the waveform morphology, but not to resolve a precise 3D site on the heart surface. So in practice the origin is found invasively, by threading mapping catheters into the chambers during the procedure and sampling the surface point by point, one chamber at a time.',
            'La herramienta que se usa para leer la electricidad del corazon, el ECG estandar de 12 derivaciones, no puede encontrarla. Doce derivaciones son en realidad solo ocho senales independientes: una proyeccion de muy baja dimension de un campo electrico tridimensional sobre unos pocos puntos de la piel. Eso basta para diagnosticar que existe una arritmia y adivinar una region amplia de origen por la morfologia de la onda, pero no para resolver un sitio 3D preciso en la superficie del corazon. Asi que en la practica el origen se encuentra de forma invasiva, introduciendo cateteres de mapeo en las camaras durante el procedimiento y muestreando la superficie punto por punto, una camara a la vez.')}</p>
          <p>{pick(lang,
            'Electrocardiographic imaging (ECGi) tries to recover what the 12-lead throws away: it records a dense body-surface potential map from a multi-electrode vest (Medtronic FDA-cleared CardioInsight system), fuses it with the patient-specific heart-torso geometry from a CT, and solves the inverse problem of electrocardiography to reconstruct potentials, electrograms, and activation maps across the whole heart surface from a single beat, non-invasively and panoramically. Early human work localized an arrhythmic focus to roughly 10 mm (Ramanathan et al., Nature Medicine 2004).',
            'La imagen electrocardiografica (ECGi) intenta recuperar lo que el ECG de 12 derivaciones descarta: registra un mapa denso de potencial de superficie corporal desde un chaleco multielectrodo (el sistema CardioInsight de Medtronic, autorizado por la FDA), lo fusiona con la geometria corazon-torso especifica del paciente de una tomografia, y resuelve el problema inverso de la electrocardiografia para reconstruir potenciales, electrogramas y mapas de activacion en toda la superficie del corazon desde un solo latido, de forma no invasiva y panoramica. El trabajo humano temprano localizo un foco arritmico a unos 10 mm (Ramanathan et al., Nature Medicine 2004).')}</p>
          <p>{pick(lang,
            'The honest catch is that this inverse problem is ill-posed: many different heart-surface sources produce almost the same body-surface signal, so tiny errors in geometry, segmentation, or noise blow up in the reconstruction, and regularization buys stability by sacrificing spatial detail. When ECGi has been checked directly against invasive contact maps in patients, agreement was moderate at best and highly variable: mean activation-time errors around 20 ms, overall correlation near zero (per-patient values from -0.68 to 0.82), and epicardial breakthrough sites mislocated by tens of millimeters, worst over scar, which is exactly the tissue ablation cares about (Duchateau et al., Heart Rhythm 2019). This is why validation cannot rest on patients alone.',
            'La pega honesta es que este problema inverso esta mal planteado: muchas fuentes distintas de superficie cardiaca producen casi la misma senal de superficie corporal, asi que errores minimos de geometria, segmentacion o ruido estallan en la reconstruccion, y la regularizacion compra estabilidad sacrificando detalle espacial. Cuando el ECGi se ha contrastado directamente con mapas de contacto invasivos en pacientes, la concordancia fue moderada en el mejor caso y muy variable: errores de tiempo de activacion en torno a 20 ms, correlacion global cercana a cero (valores por paciente de -0.68 a 0.82), y sitios de irrupcion epicardica mal ubicados por decenas de milimetros, peor sobre cicatriz, que es justo el tejido que le importa a la ablacion (Duchateau et al., Heart Rhythm 2019). Por eso la validacion no puede descansar solo en pacientes.')}</p>
          <Callout>
            {pick(lang,
