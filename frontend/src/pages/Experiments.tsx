...
const DS_NAME: Record<string, [string, string]> = {
  'human-tank': ['Human heart, torso tank (Utah)', 'Corazón humano, tanque de torso (Utah)'],
  'dog-insitu': ['Dog heart, in situ (Maastricht)', 'Corazón de perro, in situ (Maastricht)'],
};
  ...
  return (
    <div className="fig-svg wide">
      <svg viewBox="0 0 720 220" role="img" style={{ width: '100%', height: 'auto' }}>
        ...
        <text x="492" y="82" fill="var(--fg)" fontSize="12" fontWeight="600">{pick(lang, 'Heart cage (held out)', 'Jaula cardíaca (reservada)')}</text>
        ...
        <text x="492" y="136" fill="var(--good)" fontSize="10">{pick(lang, 'gain + oracle λ (not the pattern)', 'ganancia + λ oráculo (no el patrón)')}</text>
        <line x1="272" y1="200" x2="704" y2="200" stroke="var(--bad)" strokeWidth="1.4" strokeDasharray="5 3" />
        <text x="278" y="196" fill="var(--bad)" fontSize="10" textDecoration="line-through">{pick(lang, 'Forbidden: fit the recovered pattern on the held-out cage (leakage)', 'Prohibido: ajustar el patrón recuperado sobre la jaula reservada (fuga)')}</text>
        <defs>
      ...
      <div className="fig-cap">{pick(lang, 'The recovered spatial pattern is inferred from the body surface only, and correlation is scale-free. The cage truth calibrates two disclosed scalars, a magnitude gain and the oracle-best lambda, which set overall scale and smoothness but not the pattern; fitting the pattern itself on the cage is the forbidden anti-pattern, struck out.', 'El patrón espacial recuperado se infiere solo de la superficie corporal, y la correlación no depende de la escala. La verdad de la jaula calibra dos escalares declarados, una ganancia de magnitud y el mejor lambda por oráculo, que fijan la escala y la suavidad pero no el patrón; ajustar el patrón mismo sobre la jaula es el anti-patrón prohibido, tachado.')}</div>
    </div>
  );
}
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Cada experimento ECGi registró los potenciales de superficie corporal y cardíaca simultáneamente durante el latido. El patrón espacial recuperado de la reconstrucción proviene solo de los datos de superficie corporal a través del inverso; nunca se ajusta sobre la jaula. Los cuadros o derivaciones marcados malos (NaN) se descartan. Dos escalares se calibran contra la jaula reservada y se declaran abiertamente: una ganancia de magnitud única (ajustada en los potenciales de la primera mitad de la jaula; fija la escala global, así que la correlación, que no depende de la escala, no se ve afectada y solo el error relativo depende de ella), y el mejor lambda por oráculo usado en las tablas (minimiza el error real, una cota superior reportada para que cada método se juzgue en su mejor versión). El mismo pipeline se ejecuta en cada dataset sin reajuste, así que los números miden un método, no un ajuste a una geometría.')}</p>
          <ProtocolSvg lang={lang} />
          <Callout>{pick(lang, 'The oracle-best lambda used in the tables is an upper bound reported to compare methods fairly (every method judged at its best); it is not claimed as a blind operating point. Reporting it is stated openly, not hidden.', 'El mejor lambda por oráculo usado en las tablas es una cota superior reportada para comparar métodos con justicia (cada método en su mejor versión); no se afirma como un punto de operación ciego. Reportarlo se declara abiertamente, no se oculta.')}</Callout>
          <Refs ids={['aras2015', 'cluitmans2018']} label="Refs" />
        </section>
      ),
    },
    {
      id: 'metrics', label: pick(lang, 'Metrics', 'Métricas'),
      content: (
        <section>
          <h2>{pick(lang, 'The exact metrics', 'Las métricas exactas')}</h2>
          <p>{pick(lang, 'Two spatial metrics are computed per time frame and averaged over the beat, plus a calibration metric for the uncertainty.', 'Se computan dos métricas espaciales por cuadro de tiempo y se promedian sobre el latido, más una métrica de calibración para la incertidumbre.')}</p>
          <Equation tex={String.raw`\mathrm{RE}=\frac{\lVert\hat\phi-\phi^{\text{true}}\rVert_2}{\lVert\phi^{\text{true}}\rVert_2}, \qquad \mathrm{CC}=\frac{\langle\hat\phi-\bar{\hat\phi},\,\phi^{\text{true}}-\bar\phi^{\text{true}}\rangle}{\lVert\hat\phi-\bar{\hat\phi}\rVert\,\lVert\phi^{\text{true}}-\bar\phi^{\text{true}}\rVert}`}
            caption={pick(lang, 'Relative error (0 = perfect) and the spatial correlation coefficient (1 = perfect), the standard ECGi pair.', 'Error relativo (0 = perfecto) y el coeficiente de correlación espacial (1 = perfecto), el par estándar de ECGi.')} />
          <Equation tex={String.raw`\text{node-UQ} = \frac{1}{N}\sum_i \mathbb{1}\!\left[\,|\hat\phi_i-\phi^{\text{true}}_i| \le 2\,s_i\,\right], \qquad s_i=\tau\cdot\mathrm{std}_k\,\hat\phi_i^{(k)}`}
            caption={pick(lang, 'The uncertainty reliability: the fraction of nodes whose true error is inside the recalibrated 2-sigma band; ~0.90 is well-calibrated.', 'La confiabilidad de la incertidumbre: la fracción de nodos cuyo error real está dentro de la banda 2-sigma recalibrada; ~0.90 está bien calibrado.')} />
          <Callout>{pick(lang, 'For 4D-flow the analogous metric is the recovered pressure range vs the clinical Bernoulli estimate 4·Vmax² and the analytic-gate error, since no per-voxel pressure truth exists.', 'Para el flujo 4D la métrica análoga es el rango de presión recuperado vs la estimación clínica de Bernoulli 4·Vmax² y el error del gate analítico, ya que no existe verdad de presión por vóxel.')}</Callout>
    ...
    },
    {
      ...
      content: (
        <section>
          ...
          <p>{pick(lang, 'Read directly from the committed catalogue artifact (never typed in). Two species, two labs, diffuse (sinus) and focal (paced) activation, one pipeline.', 'Leído directamente del artefacto de catálogo comprometido (nunca escrito a mano). Dos especies, dos laboratorios, activación difusa (sinusal) y focal (marcapaso), un pipeline.')}</p>
          {!cat ? <div className="panel">Loading...</div> : (
            <div className="overflow-x">
              <table>
                <thead><tr><th>{pick(lang, 'Dataset', 'Conjunto')}</th><th>{pick(lang, 'Beat', 'Latido')}</th><th>{pick(lang, 'Body -> heart', 'Cuerpo -> corazón')}</th><th>RE</th><th>CC</th><th>Node-UQ (2σ)</th><th>{pick(lang, 'Frames', 'Cuadros')}</th></tr></thead>
                <tbody>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Un escaneo de flujo 4D no tiene patrón de oro de presión invasivo (la razón del método), así que la recuperación de presión se valida de tres formas. Primero, una prueba analítica: en flujos de respuesta exacta conocida (un ducto convergente para la presión estacionaria, un Poiseuille variable en el tiempo para el término no estacionario) el motor los recupera (correlación estacionaria 1.00, dv/dt no estacionario correlación 0.995) antes de confiar en datos reales. Segundo, el rango fisiológico en el escaneo real. Tercero, la estimación clínica de Bernoulli del mismo escaneo como referencia.')}</p>
          {flow && (
            <div className="overflow-x">
              <table>
                ...
                <tbody>
                  ...
                  <tr><td>{pick(lang, 'PPE relative-pressure range', 'Rango de presión relativa PPE')}</td><td className="mono">{flow.metrics.ppe_pressure_drop_mmHg} mmHg</td></tr>
                  <tr><td>{pick(lang, 'Clinical Bernoulli 4·Vmax²', 'Bernoulli clínico 4·Vmax²')}</td><td className="mono">{flow.metrics.bernoulli_mmHg} mmHg</td></tr>
                  ...
                  <tr><td>{pick(lang, 'Lumen voxels resolved', 'Vóxeles de lumen resueltos')}</td><td className="mono">{flow.metrics.n_lumen_voxels}</td></tr>
                </tbody>
              </table>
            </div>
          )}
          <Callout>{pick(lang, 'The recovered range (0.79 mmHg) is small and physiological for this unobstructed aorta and the same order as Bernoulli (2.51 mmHg). A velocity-noise ensemble moves it under 0.01 mmHg, so the dominant uncertainty is the absent gold standard, not measurement noise; the absolute magnitude carries the method uncertainty honestly.', 'El rango recuperado (0.79 mmHg) es pequeño y fisiológico para esta aorta sin obstrucción y del mismo orden que Bernoulli (2.51 mmHg). Un ensemble de ruido lo mueve menos de 0.01 mmHg, así que la incertidumbre dominante es el patrón de oro ausente, no el ruido; la magnitud absoluta lleva la incertidumbre del método con honestidad.')}</Callout>
    ...
    },
    {
      ...
      content: (
        <section>
          ...
          <div className="overflow-x">
            <table>
              <thead><tr><th>{pick(lang, 'Dataset', 'Conjunto')}</th><th>{pick(lang, 'Case', 'Caso')}</th><th>{pick(lang, 'Source', 'Fuente')}</th><th>{pick(lang, 'Redistribution', 'Redistribución')}</th><th>{pick(lang, 'Status', 'Estado')}</th></tr></thead>
              <tbody>
                ...
                <tr><td>{pick(lang, 'Thoracic-aorta 4D-flow', 'Flujo 4D de aorta torácica')}</td><td>4D-flow</td><td className="small">{pick(lang, 'real Philips MRI (venc 120)', 'MRI Philips real (venc 120)')}</td><td className="small">{pick(lang, 'link-only (gitignored)', 'solo enlace (gitignored)')}</td><td className="good">{pick(lang, 'integrated', 'integrado')}</td></tr>
                ...
                <tr><td>Valencia</td><td>ECGi</td><td className="small">EDGAR / CEI</td><td className="small">-</td><td className="muted small">{pick(lang, 'excluded (simulation)', 'excluido (simulación)')}</td></tr>
    ...
    <div className="page-body prose">
      <div className="page-head">
        ...
        <p className="lede">{pick(lang,
          ...
          'El diseño de validación y los resultados reales para ambos casos. Para la imagen de ECG cada número es la calidad de reconstrucción medida contra los potenciales reales de superficie cardíaca en un catálogo de experimentos EDGAR independientes; para el flujo 4D, donde no existe verdad de presión invasiva, la validación es la prueba analítica, el rango fisiológico, y el encuadre clínico de Bernoulli. Nada se valida contra un campo sintético.')}</p>
      </div>
