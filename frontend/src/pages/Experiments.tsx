  ...
  return (
    <div className="fig-svg wide">
      <svg viewBox="0 0 720 220" role="img" style={{ width: '100%', height: 'auto' }}>
        ...
        <text x="30" y="160" fill="var(--muted)" fontSize="9.5">{pick(lang, 'the reconstruction sees', 'la reconstruccion ve')}</text>
        ...
        <text x="492" y="100" fill="var(--muted)" fontSize="10">{pick(lang, 'true potentials, used ONLY', 'potenciales reales, usados SOLO')}</text>
        <text x="492" y="114" fill="var(--muted)" fontSize="10">{pick(lang, 'to compute RE / CC / UQ', 'para calcular RE / CC / UQ')}</text>
        <text x="492" y="136" fill="var(--good)" fontSize="10">{pick(lang, 'never seen by the inverse', 'nunca vista por el inverso')}</text>
        <line x1="272" y1="200" x2="704" y2="200" stroke="var(--bad)" strokeWidth="1.4" strokeDasharray="5 3" />
        <text x="278" y="196" fill="var(--bad)" fontSize="10" textDecoration="line-through">{pick(lang, 'FORBIDDEN: fit λ or the gain on the held-out cage (leakage)', 'PROHIBIDO: ajustar λ o la ganancia sobre la jaula reservada (fuga)')}</text>
        <defs>
      ...
      <div className="fig-cap">{pick(lang, 'Leakage-safe design: the inverse sees only the body surface; the heart cage is held out and used solely to score. The forbidden anti-pattern (tuning any parameter on the held-out cage) is struck out.', 'Diseno sin fuga: el inverso ve solo la superficie corporal; la jaula se reserva y se usa solo para puntuar. El anti-patron prohibido (ajustar cualquier parametro sobre la jaula reservada) esta tachado.')}</div>
    </div>
  );
}
    ...
    <div className="page-body prose">
      ...
      {tab === 'protocol' && (
        <section>
          ...
          <p>{pick(lang,
            'Each ECGi experiment recorded the body-surface and the heart-surface potentials simultaneously over the beat. The reconstruction sees ONLY the body-surface data; the heart-surface cage recording is held out and used solely to score the result. Time frames or leads flagged bad (NaN) are dropped. The one scalar forward gain is calibrated on the first half of the beat only, so the forward model is never tuned on the evaluation window, and no regularization parameter is ever fit on the held-out cage. The identical pipeline runs on every dataset with no per-heart retuning, so the numbers measure a method, not a fit to one geometry.',
      ...
      {tab === 'metrics' && (
        <section>
          ...
          <Equation tex={String.raw`\mathrm{RE}=\frac{\lVert\hat\phi-\phi^{\text{true}}\rVert_2}{\lVert\phi^{\text{true}}\rVert_2}, \qquad \mathrm{CC}=\frac{\langle\hat\phi-\bar{\hat\phi},\,\phi^{\text{true}}-\bar\phi^{\text{true}}\rangle}{\lVert\hat\phi-\bar{\hat\phi}\rVert\,\lVert\phi^{\text{true}}-\bar\phi^{\text{true}}\rVert}`}
            caption={pick(lang, 'Relative error (0 = perfect) and the spatial correlation coefficient (1 = perfect), the standard ECGi pair.', 'Error relativo (0 = perfecto) y el coeficiente de correlacion espacial (1 = perfecto), el par estandar de ECGi.')} />
      ...
      {tab === 'flow-proto' && (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Un escaneo de flujo 4D no tiene patron de oro de presion invasivo (la razon del metodo), asi que la recuperacion de presion se valida de tres formas. Primero, una prueba analitica: en flujos de respuesta exacta conocida (un ducto convergente para la presion estacionaria, un Poiseuille variable en el tiempo para el termino no estacionario) el motor los recupera (correlacion estacionaria 1.00, dv/dt no estacionario correlacion 0.995) antes de confiar en datos reales. Segundo, el rango fisiologico en el escaneo real. Tercero, la estimacion clinica de Bernoulli del mismo escaneo como referencia.')}</p>
          {flow && (
            <div className="overflow-x">
              <table>
