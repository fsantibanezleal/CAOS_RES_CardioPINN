      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Cada experimento ECGi registro los potenciales de superficie corporal y cardiaca simultaneamente durante el latido. El PATRON espacial recuperado de la reconstruccion proviene solo de los datos de superficie corporal a traves del inverso; nunca se ajusta sobre la jaula. Los cuadros o derivaciones marcados malos (NaN) se descartan. Dos escalares se calibran contra la jaula reservada y se declaran abiertamente: una ganancia de magnitud unica (ajustada en los potenciales de la primera mitad de la jaula; fija la escala global, asi que la correlacion, que no depende de la escala, no se ve afectada y solo el error relativo depende de ella), y el mejor lambda por oraculo usado en las tablas (minimiza el error real, una cota SUPERIOR reportada para que cada metodo se juzgue en su mejor version). El mismo pipeline corre en cada dataset sin reajuste, asi que los numeros miden un metodo, no un ajuste a una geometria.')}</p>
