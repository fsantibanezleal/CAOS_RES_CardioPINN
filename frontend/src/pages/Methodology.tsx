...
function ForwardSvg({ lang }: { lang: Lang }) {
  return (
    <Fig lang={lang} caption={['The torso as a volume conductor: a heart source maps to the body surface by A; A’s singular values decay to zero (right), the ill-conditioning.', 'El torso como conductor de volumen: una fuente cardíaca se mapea a la superficie por A; los valores singulares de A decaen a cero (derecha), el mal condicionamiento.']}>
...
function TikhonovSvg({ lang }: { lang: Lang }) {
  return (
    <Fig lang={lang} caption={['The L-curve: solution norm vs data misfit as λ varies; the corner is the classical balance between fitting the data and staying stable.', 'La curva L: norma de la solución vs desajuste a datos al variar λ; la esquina es el balance clásico entre ajustar los datos y mantener estabilidad.']}>
...
function ParamSvg({ lang }: { lang: Lang }) {
  return (
    <Fig lang={lang} caption={['The reconstruction error as a function of λ is U-shaped; we report the oracle minimum (fair to every method) rather than an L-curve heuristic.', 'El error de reconstrucción en función de λ tiene forma de U; reportamos el mínimo por oráculo (justo para todo método) en lugar de una heurística de curva L.']}>
      <line x1="70" y1="30" x2="70" y2="165" {...AX} /><line x1="70" y1="165" x2="520" y2="165" {...AX} />
...
function UqSvg({ lang }: { lang: Lang }) {
  return (
    <Fig lang={lang} caption={['Calibration: after temperature recalibration the empirical coverage tracks the nominal (dashed identity); ~90% of nodes fall within the 2σ band of the true error.', 'Calibración: tras recalibrar la temperatura la cobertura empírica sigue la identidad (línea punteada); ~90% de los nodos caen dentro de la banda 2σ del error real.']}>
  ...
  const ecgiTabs: SubTabDef[] = [
    {
      ...
      content: (
        <section>
          <h2>{pick(lang, 'The forward operator: torso volume conduction', 'El operador directo: conducción de volumen del torso')}</h2>
          <p>{pick(lang,
            ...
            'El problema directo mapea una distribución dada de potencial de superficie cardíaca a los potenciales que produce en la superficie corporal. En el régimen cuasiestático del latido el potencial extracelular es armónico en el torso, así que el mapa es lineal y está codificado por una sola matriz de transferencia A que depende solo de la geometría y las conductividades del tejido, no de la señal.')}</p>
          <p>{pick(lang,
            ...
            'El operador que esta implementación realmente calcula e invierte es una aproximación de fuente puntual en medio no acotado (capa simple): A mapea cada nodo de la jaula cardíaca a cada electrodo corporal por el recíproco de su distancia, suavizado y normalizado por filas para el condicionamiento, con una ganancia escalar calibrada en la primera mitad del latido, luego fijada y aplicada a toda la matriz. Se evalúa sobre las posiciones reales de los electrodos del experimento EDGAR, no sobre una malla genérica. Un operador de elementos de contorno homogéneo también está implementado y probado analíticamente en el problema de esferas concéntricas (ver la pestaña Pruebas analíticas), pero las superficies de electrodos de EDGAR son nubes de puntos abiertas, no las 2-variedades cerradas que un BEM necesita, así que la reconstrucción recae en el operador de capa simple y el BEM se ejecuta solo como comparación offline en geometrías de catálogo de variedad cerrada. El BEM completo de torso cerrado con conductividades por tejido es el siguiente paso natural.')}</p>
          ...
          <Callout>{pick(lang,
            ...
            'Aproximación de capa simple, no BEM completo; limita la precisión absoluta pero es honesta y autocontenida. La geometría real de electrodos (192 torso, 256 nodos de jaula + triangulaciones) se usa exactamente.')}</Callout>
    ...
    },
    {
      ...
      content: (
        <section>
          <h2>{pick(lang, 'Tikhonov regularization', 'Regularización de Tikhonov')} <Cite id="tikhonov1977" /></h2>
          <p>{pick(lang,
            ...
            'Resolver directamente A phi = phi_body es inestable porque los valores singulares pequeños de A amplifican el ruido sin límite. La regularización de Tikhonov reemplaza el inverso inestable por una minimización bien planteada: ajustar los datos y mantener la solución controlada, ponderando ambos con un parámetro lambda.')}</p>
          <p>{pick(lang,
            ...
            'La minimización tiene forma cerrada, un solo sistema lineal, lo que hace a Tikhonov rápido y el baseline estándar de investigación clínica. Su costo es un sesgo: la penalización suaviza la solución, difuminando los frentes de activación agudos, y devuelve una sola estimación sin incertidumbre. Esta implementación computa la reconstrucción de Tikhonov de orden cero por cuadro de tiempo.')}</p>
          ...
          <Callout>{pick(lang,
            ...
            'Las variantes L1 / de variación total agudizan los frentes pero siguen siendo deterministas. Mantenemos el Tikhonov L2 clásico como el baseline honesto y bien entendido y le damos su mejor lambda por oráculo (pestaña Elección del parámetro).')}</Callout>
    ...
    {
      id: 'param', label: pick(lang, 'Parameter choice', 'Elección del parámetro'),
      content: (
        <section>
          <h2>{pick(lang, 'Choosing the regularization strength', 'Elección de la fuerza de regularización')} <Cite id="hansen1992" /></h2>
          <p>{pick(lang,
            ...
            'Lambda controla el compromiso entre fidelidad a los datos y estabilidad de la solución: muy pequeño y el ruido domina, muy grande y la reconstrucción se sobre-suaviza a casi cero. Clásicamente se elige por la curva L, un gráfico log-log de la norma de la solución contra la norma del residuo al variar lambda, cuya esquina marca el mejor compromiso, o por los criterios CRESO y GCV.')}</p>
          <p>{pick(lang,
            ...
            'Como este experimento tiene los potenciales verdaderos de superficie cardíaca, comparamos los métodos de forma justa dando a cada uno su mejor lambda por oráculo, el valor en un barrido logarítmico que minimiza el error real de reconstrucción. Esto juzga el baseline clásico en su mejor versión; un pipeline clínico sin la verdad de referencia usaría la curva L, que típicamente está cerca del oráculo.')}</p>
          <Equation tex={String.raw`\lambda^\star = \arg\min_{\lambda}\; \frac{\lVert \hat\phi(\lambda) - \phi_{\text{heart}}^{\text{measured}}\rVert}{\lVert \phi_{\text{heart}}^{\text{measured}}\rVert} \quad\text{(oracle, real ground truth available)}`}
            caption={pick(lang, 'The oracle criterion used for a fair comparison; the clinical L-curve corner approximates it without the truth.', 'El criterio de oráculo usado para una comparación justa; la esquina de la curva L clínica lo aproxima sin la verdad.')} />
          <Callout>{pick(lang,
            ...
            'El lambda de oráculo usa la verdad de referencia solo para elegir el baseline más justo, nunca para ajustar la reconstrucción. La reconstrucción en sí solo ve los datos de superficie corporal.')}</Callout>
          <ParamSvg lang={lang} />
    ...
    },
    {
      ...
      content: (
        <section>
          <h2>{pick(lang, 'A spatial prior on the real heart surface', 'Un prior espacial sobre la superficie cardíaca real')}</h2>
          <p>{pick(lang,
            ...
            'Una penalización genérica de magnitud ignora que los potenciales de superficie cardíaca son un campo suave sobre una superficie curva: los electrodos vecinos deberían tener valores similares porque el tejido es continuo. Codificamos eso penalizando el Laplaciano de grafo de la triangulación real de la jaula cardíaca, que mide cuánto difiere cada nodo de sus vecinos en la malla.')}</p>
          <p>{pick(lang,
            ...
            'Esto reemplaza la penalización de identidad por una consciente de la geometría construida a partir de la conectividad real de la malla, así que la suavidad se impone a lo largo del tejido en lugar de en un espacio de coordenadas abstracto. Es el análogo discreto de penalizar el gradiente superficial del potencial.')}</p>
          <Equation tex={String.raw`\hat\phi = (A^{\top}A + \lambda^2 L_{\text{mesh}}^{\top}L_{\text{mesh}})^{-1}A^{\top}\phi_{\text{body}}, \qquad (L_{\text{mesh}})_{ij} = \deg(i)\,[i{=}j] - [\,i\sim j\,]`}
            caption={pick(lang, 'The graph-Laplacian-regularized inverse; L_mesh is the combinatorial Laplacian of the cage triangulation.', 'El inverso regularizado por Laplaciano de grafo; L_mesh es el Laplaciano combinatorio de la triangulación de la jaula.')} />
          <Callout>{pick(lang,
            ...
            'Sobre estos datos el prior de grafo iguala al Tikhonov de orden cero en error relativo y cambia ligeramente la correlación; su valor es una suavidad físicamente significativa que además estabiliza la incertidumbre por ensemble. Un Laplaciano cotangente (métrico) sería un refinamiento.')}</Callout>
          <PriorSvg lang={lang} />
    ...
    },
    {
      ...
      content: (
        <section>
          <h2>{pick(lang, 'Physics-constrained reconstruction', 'Reconstrucción restringida por física')} <Cite id="raissi2019" /></h2>
          <p>{pick(lang,
            ...
            'La visión informada por física es que el campo recuperado debe ser consistente con el operador gobernante: cualesquiera potenciales de superficie cardíaca que emitamos, proyectarlos hacia adelante a través de A debe reproducir los datos reales medidos de superficie corporal. Ese término de consistencia con datos es la restricción física; el prior espacial es el regularizador; juntos definen el objetivo que minimizamos.')}</p>
          <p>{pick(lang,
            ...
            'Este es el mismo principio que una red neuronal informada por física, imponer la ecuación gobernante como restricción suave mientras los datos medidos guían el ajuste, aquí aplicado al problema inverso lineal para que la solución siga siendo un mínimos cuadrados rápido y bien entendido. La literatura de PINN cardíaco (Sahli Costabal 2020) aplica la misma idea al problema no lineal de activación Eikonal.')}</p>
          <Equation tex={String.raw`\mathcal{L}(\phi) = \underbrace{\lVert A\phi - \phi_{\text{body}}^{\text{measured}}\rVert_2^2}_{\text{physics / data consistency}} + \underbrace{\lambda^2\lVert L_{\text{mesh}}\phi\rVert_2^2}_{\text{surface prior}}`}
            caption={pick(lang, 'The physics-constrained objective: forward-consistency with the real measurement plus the geometry-aware prior.', 'El objetivo restringido por física: consistencia directa con la medición real más el prior consciente de la geometría.')} />
          <Callout>{pick(lang,
            ...
            'Como el operador aquí es lineal, la solución restringida por física equivale al mínimos cuadrados regularizado; el encuadre PINN importa más para los problemas cardíacos no lineales (activación, flujo) donde la restricción es un residuo de PDE evaluado por autograd.')}</Callout>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Una sola reconstrucción no puede decir dónde debe confiarse, lo cual para un inverso clínico mal planteado es esencial: el clínico debe saber qué partes del mapa son confiables. Estimamos esto por un ensemble profundo: repetir la reconstrucción muchas veces, cada una sobre los datos medidos perturbados por una realización independiente de ruido de medición realista, y tomar la dispersión en el ensemble como la incertidumbre por nodo.')}</p>
          <p>{pick(lang,
            ...
            'Las dispersiones crudas del ensemble suelen estar mal calibradas (sobre o sub-confiadas). Recalibramos con una sola temperatura para que la banda promedio de dos desviaciones estándar iguale el error promedio real; sobre estos datos la confiabilidad, la fracción de nodos cuyo error real cae dentro de dos desviaciones estándar, sube a cerca de 0.90. Ese mapa calibrado es el valor agregado concreto sobre una sola estimación de Tikhonov.')}</p>
          <Equation tex={String.raw`\bar\phi = \tfrac1K\sum_{k=1}^{K}\hat\phi^{(k)}, \quad s_0 = \operatorname{std}_k\hat\phi^{(k)}, \quad \tau = \frac{\overline{|\bar\phi-\phi^{\text{true}}|}\,\sqrt{\pi/2}}{\overline{s_0}}, \quad s = \tau\,s_0`}
            caption={pick(lang, 'Ensemble mean, raw spread, the recalibration temperature, and the calibrated per-node uncertainty s.', 'Media del ensemble, dispersión cruda, la temperatura de recalibración, y la incertidumbre por nodo calibrada s.')} />
          <Callout>{pick(lang,
            ...
            'La recalibración usa el error real solo para fijar un escalar global; no ajusta la reconstrucción. Una calibración totalmente ciega (validación cruzada en la superficie corporal) es la versión clínica honesta y un siguiente paso.')}</Callout>
          <UqSvg lang={lang} />
  ...
  const flowTabs: SubTabDef[] = [
    {
      ...
      content: (
        <section>
          <h2>{pick(lang, 'From Navier-Stokes to the pressure-Poisson equation', 'De Navier-Stokes a la ecuación de Poisson de presión')}</h2>
          <p>{pick(lang,
            ...
            'La sangre en una arteria grande es un fluido newtoniano incompresible, así que su movimiento obedece las ecuaciones de Navier-Stokes incompresibles: conservación del momento, que liga el gradiente de presión a la aceleración del fluido y la fricción viscosa, más la restricción de incompresibilidad (divergencia cero). La presión aparece solo por su gradiente, y nunca se mide.')}</p>
          <p>{pick(lang,
            ...
            'Tomar la divergencia de la ecuación de momento y usar la incompresibilidad elimina el término de aceleración y convierte la relación en una sola ecuación elíptica de Poisson para la presión, cuyo lado derecho se construye enteramente a partir de la velocidad medida y sus derivadas espaciales. Es un problema bien planteado, a diferencia de una inversión ingenua; toda la dificultad se traslada a calcular una fuente limpia desde una velocidad medida ruidosa.')}</p>
          <Equation tex={String.raw`\rho\big(\partial_t\mathbf{v}+(\mathbf{v}\cdot\nabla)\mathbf{v}\big)=-\nabla p+\mu\nabla^2\mathbf{v},\qquad \nabla\cdot\mathbf{v}=0`}
            caption={pick(lang, 'Incompressible Navier-Stokes: momentum balance plus zero divergence. rho = 1060 kg/m3, mu = 0.0035 Pa s.', 'Navier-Stokes incompresible: balance de momento más divergencia cero. rho = 1060 kg/m3, mu = 0.0035 Pa s.')} />
          <Equation tex={String.raw`\nabla^2 p = -\rho\sum_{i,j}\frac{\partial v_i}{\partial x_j}\frac{\partial v_j}{\partial x_i}\equiv S(\mathbf{v})`}
            caption={pick(lang, 'The pressure-Poisson equation: the source S is a quadratic form of the velocity gradients, so noise in v is amplified, hence the divergence-free denoising.', 'La ecuación de Poisson de presión: la fuente S es una forma cuadrática de los gradientes de velocidad, así que el ruido en v se amplifica, de ahí el suavizado sin divergencia.')} />
          <NsSvg lang={lang} />
          <Callout>{pick(lang, 'There is no invasive pressure gold standard, which is exactly why the method exists; the pressure is forced out of the measured velocity by the physics.', 'No hay patrón de oro de presión invasivo, que es justo por lo que existe el método; la presión la fuerza la física desde la velocidad medida.')}</Callout>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Como la fuente de la Poisson de presión es un producto de derivadas de velocidad, el ruido de medición crudo (que viola la incompresibilidad) se amplifica en una presión no fisiológica. La solución es un paso de velocidad informado por física: una red se entrena para reproducir la velocidad medida satisfaciendo divergencia cero en puntos de colocación, produciendo un campo suave y sin divergencia cuyas derivadas analíticas son limpias.')}</p>
          <p>{pick(lang,
            ...
            'A diferencia de la presión, la velocidad está fuertemente restringida por los datos, así que este suavizado está bien planteado y es robusto. Una red simple de residuo de momento que intenta emitir presión directamente no puede recuperarla a Reynolds aórtico (la presión no tiene calibre y se acopla débilmente); ese enfoque fallido se conserva como baseline documentado. Separar el ajuste de velocidad bien planteado de la resolución elíptica es lo que funciona.')}</p>
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'La aceleración no estacionaria domina la presión en sístole pico. Una red espacio-temporal v(x,y,z,t) se entrena sin divergencia sobre todo el ciclo cardíaco, así que la fuente de la Poisson y la aceleración no estacionaria dv/dt son ambas analíticas (derivadas exactamente en el tiempo por autograd), reemplazando una diferencia finita ruidosa de tres cuadros.')}</p>
          <p>{pick(lang,
            ...
            'En el escaneo real el término analítico lleva el rango de presión relativa de 14.87 mmHg inflado (la diferencia finita ruidosa) a un pequeño y fisiológico 0.79 mmHg, del mismo orden que la estimación clínica de Bernoulli (2.51 mmHg) del mismo escaneo. Es una mejora real medida sobre los datos, no una elección de ajuste.')}</p>
          <Equation tex={String.raw`\partial_t\mathbf{v}=\frac{U}{T}\,\partial_{\tilde t}\,\mathbf{v}_\theta(\tilde x,\tilde t)\quad(\text{analytic, autograd in the non-dimensional time})`}
            caption={pick(lang, 'The analytic temporal derivative via the chain rule through the non-dimensional time; it feeds the full Neumann flux with the exact unsteady contribution.', 'La derivada temporal analítica por la regla de la cadena en el tiempo adimensional; alimenta el flujo Neumann completo con la contribución no estacionaria exacta.')} />
          <Equation tex={String.raw`\mathbf{b}=\mathbf{b}_{\text{steady}}-\rho\,\partial_t\mathbf{v}`}
            caption={pick(lang, 'The full Neumann wall flux = the steady (convective + viscous) part minus the analytic unsteady acceleration.', 'El flujo Neumann completo = la parte estacionaria (convectiva + viscosa) menos la aceleración no estacionaria analítica.')} />
      ...
      content: (
        <section>
          ...
          <p>{pick(lang,
            ...
            'Con una fuente analítica limpia y un flujo Neumann de frontera del campo suavizado, la presión es la solución de una ecuación de Poisson sobre el lumen aórtico segmentado con condición Neumann en la pared y un pin de Dirichlet (la presión se define salvo constante). Se resuelve por un método directo disperso en la mayor componente conexa, robusto donde un solver iterativo se estanca en el lumen real irregular y mal condicionado.')}</p>
          <p>{pick(lang,
            ...
            'Calcular la fuente S y el flujo Neumann b a partir de las derivadas analíticas de la red (en lugar de diferencias finitas en el borde) es el paso decisivo: el borde, donde un campo de rejilla encuentra el exterior en cero, es donde las diferencias finitas fabrican los peores artefactos. Hacerlo analíticamente es lo que lleva el mapa de miles de mmHg no fisiológicos a un rango fisiológico.')}</p>
          <Equation tex={String.raw`\nabla^2 p = S(\mathbf{v}_\theta) \text{ in } \Omega_{\text{lumen}},\qquad \partial_n p = \mathbf{b}(\mathbf{v}_\theta)\cdot\mathbf{n} \text{ on } \partial\Omega,\qquad p(x_0)=0`}
            caption={pick(lang, 'The elliptic boundary-value problem for the relative pressure: Poisson interior, Neumann wall, one Dirichlet pin to fix the additive constant.', 'El problema elíptico de valores de contorno para la presión relativa: Poisson interior, Neumann en la pared, un pin de Dirichlet para fijar la constante aditiva.')} />
          <Equation tex={String.raw`\text{sparse system } L\,\mathbf{p}=\mathbf{f},\qquad L=\text{7-point Laplacian on the lumen voxels}`}
            caption={pick(lang, 'Discretized as a sparse linear system on the lumen voxels and solved by a direct factorization.', 'Discretizado como un sistema lineal disperso sobre los vóxeles del lumen y resuelto por una factorización directa.')} />
          <Fig lang={lang} caption={['The recovered relative pressure on the lumen; the range is small (~0.79 mmHg) and physiological for an unobstructed aorta.', 'La presión relativa recuperada sobre el lumen; el rango es pequeño (~0.79 mmHg) y fisiológico para una aorta sin obstrucción.']}>
    ...
    {
      id: 'gate', label: pick(lang, 'Analytic gates', 'Pruebas analíticas'),
      content: (
        <section>
          <h2>{pick(lang, 'The analytic gates: proof before real data', 'Las pruebas analíticas: prueba antes de datos reales')}</h2>
          <p>{pick(lang,
            ...
            'Como no hay patrón de oro de presión invasivo, el motor se prueba en flujos analíticos de respuesta exacta conocida antes de confiar en datos reales. La resolución de presión estacionaria se prueba en un ducto convergente analítico cuya caída de presión exacta se conoce, recuperada con menos de 1 por ciento de error (correlación 1.00, 4.74 vs 4.73 mmHg). El término no estacionario se prueba en un flujo de Poiseuille variable en el tiempo cuyo dw/dt exacto se conoce, recuperado con correlación 0.995.')}</p>
          <p>{pick(lang,
            ...
            'Estas pruebas son tests de pytest que deben pasar en integración continua; solo tras pasar se aplica el pipeline al escaneo real. El operador directo BEM homogéneo del caso ECGi se prueba igual, en el problema de esferas concéntricas donde la transferencia armónica se conoce en forma cerrada (correlación 1.00, error relativo que decrece con cada refinamiento de malla); como las superficies de electrodos de EDGAR son abiertas, ese BEM se usa solo como comparación offline en geometrías de catálogo de variedad cerrada, mientras que la reconstrucción de EDGAR usa el operador de capa simple probado. Esta es la disciplina que separa un resultado real de uno de apariencia plausible.')}</p>
          <Equation tex={String.raw`\text{converging duct: } \Delta p_{\text{true}}=4.73\text{ mmHg},\;\; \Delta p_{\text{rec}}=4.74\text{ mmHg},\;\; \text{corr }1.00`}
            caption={pick(lang, 'The steady pressure gate: the known analytic drop is recovered to within 1 percent before real data is used.', 'La prueba de presión estacionaria: la caída analítica conocida se recupera con menos de 1 por ciento de error antes de usar datos reales.')} />
          <Equation tex={String.raw`\text{time-varying Poiseuille: } \tfrac{dw}{dt}\Big|_{\text{rec}}\;\text{vs}\;\tfrac{dw}{dt}\Big|_{\text{true}},\;\; \text{corr }0.995`}
            caption={pick(lang, 'The unsteady gate: the analytic acceleration is recovered before the space-time term is trusted on the real scan.', 'La prueba no estacionaria: la aceleración analítica se recupera antes de confiar en el término espacio-temporal sobre el escaneo real.')} />
          <Fig lang={lang} caption={['Every engine passes a known-answer analytic gate (green) before it is applied to the real scan; a failing gate blocks the pipeline in CI.', 'Cada motor pasa una prueba analítica de respuesta conocida (verde) antes de aplicarse al escaneo real; una prueba fallida bloquea el pipeline en CI.']}>
  ...
  return (
    <div className="page-body prose">
      <div className="page-head">
        <h1>{pick(lang, 'Methodology', 'Metodología')}</h1>
        <p className="lede">{pick(lang,
          ...
          'El panorama de métodos para ambos casos: el problema inverso de ECG (operador directo, inverso regularizado, prior espacial, ajuste restringido por física, incertidumbre calibrada) y la recuperación de presión de flujo 4D (Navier-Stokes a Poisson de presión, el suavizador sin divergencia, el término no estacionario analítico, la resolución elíptica, las pruebas analíticas). Al cambiar el dominio, cada familia es una sub-pestaña con sus ecuaciones gobernantes, las decisiones exactas de esta implementación, y sus límites honestos.')}</p>
      </div>

      <div className="row" style={{ marginBottom: 16 }}>
        <span className="muted small">{pick(lang, 'Physics domain', 'Dominio físico')}:</span>
        <span className={`chip ${dom === 'ecgi' ? 'on' : ''}`} onClick={() => setDom('ecgi')}>{pick(lang, 'ECG imaging (volume conduction)', 'Imagen de ECG (conducción de volumen)')}</span>
        <span className={`chip ${dom === 'flow' ? 'on' : ''}`} onClick={() => setDom('flow')}>{pick(lang, '4D-flow pressure (Navier-Stokes)', 'Presión de flujo 4D (Navier-Stokes)')}</span>
      </div>
